...
class PikaSelectConnection:
    ...
    def on_consumer_cancelled(self, method_frame):
        logger.info(f"Consumer was cancelled remotely, reopen consumer: {method_frame}")
    ...
    def on_delivery_confirmation(self, method_frame):
        ...
        logger.debug(f"Received {confirmation_type} for delivery tag: {method_frame.method.delivery_tag}")
    ...
    def publish_message(self, message, queue_name: str | None = None, properties: pika.BasicProperties = None):
        ...
        if queue_name == self.queue_name:
            ...
            logger.debug(f"Published message # {self._message_number}")
        else:
    ...
    def on_basic_get_empty(self, _method):
        logger.debug(f"empty queue allow try again consuming in {self._EMPTY_QUEUE_DELAY} seconds")
